...
from src.tools import AgentDeps, docs_navigator_tool
from src.rubrics_generator.visualize_rubrics import visualize_rubrics
...
def summarize_docs_tree(docs_tree: list, max_depth: int = 2) -> list:
    """Creates a shallow copy of the docs tree for prompt inclusion."""
    from typing import Any

    def _recursively_summarize(node: Any, current_depth: int) -> Any:
        if not isinstance(node, dict):
            return node # Return string directly if it's not a dictionary

        if current_depth >= max_depth:
            return {"title": node.get("title"), "path": node.get("path"), "subpages": "... (truncated)"}
        
        summarized_subpages = []
        for child in node.get("subpages", []) or []:
            summarized_subpages.append(_recursively_summarize(child, current_depth + 1))
        
        return {
            "title": node.get("title"),
            "path": node.get("path"),
            "subpages": summarized_subpages
        }

    # docs_tree is a list of top-level nodes
    summarized_list = []
    for item in docs_tree:
        summarized_list.append(_recursively_summarize(item, 1))
...
async def run(args):
    ...
    docs_path = os.path.join(base_path, "deepwiki")
    ...
    prompt = f"""
Given the docs tree (summarized):
```
...
""".strip()
    ...
    try:
        ...
        if json_match:
            try:
                rubrics_json = json_match.group(1)
                rubrics = json.loads(rubrics_json)
            except json.JSONDecodeError:
                pass # Fallback to the next method if markdown parsing fails

        if rubrics is None:
            # Fallback to finding the first '[' and last ']' if markdown parsing didn't work
