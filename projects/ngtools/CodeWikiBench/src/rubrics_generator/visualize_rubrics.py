...
class Rubric(BaseModel):
    ...
    description: str = Field(description="A clear explanation of its purpose")
    ...
    children: List["Rubric"] = Field(default_factory=list, description="The list of children rubrics. Leaf rubrics should not have children.")
...
def rubric_to_graph(rubric: Rubric) -> nx.DiGraph:
    ...
    def _add_rubric_recursive(rubric: Rubric, parent_name: str = None):
        ...
        if rubric.name in visited:
            return
        visited.add(rubric.name)
        ...
        graph.add_node(
            ...
            is_leaf=len(rubric.children) == 0,
            has_references=len(rubric.reference) > 0
        )
        
        # Add edge from parent to this rubric
        if parent_name:
            graph.add_edge(parent_name, rubric.name)
        
        # Recursively add children
        for child in rubric.children:
            _add_rubric_recursive(child, rubric.name)
...
def visualize_rubrics(path: str) -> None:
    root = Rubric(
        name="root rubric",
        description="Root of the generated rubrics",
        weight=3,
        children=[Rubric(**rubric) for rubric in rubrics]
    )
