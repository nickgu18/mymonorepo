...
from src.tools.docs_navigator import AgentDeps, docs_navigator_tool, test_run_docs_navigator
from src.utils import get_llm, run_llm_natively
...
async def evaluate_leaf_requirements(
    ...
    def summarize_docs_tree(node):
        if isinstance(node, dict):
            return {key: summarize_docs_tree(value) for key, value in node.items() if key != 'content'}
        elif isinstance(node, list):
            return [summarize_docs_tree(element) for element in node]
        else:
            return node
    ...
    async def evaluate_single_requirement(leaf):
        ...
        try:
            ...
            search_results = deps.docs_navigator.search_content(leaf['requirement'])
            
            # Get content for top 3 search results
            paths_to_get = [result['path'] for result in search_results[:3]]
            
            # Get content from docs navigator
            docs_content = await test_run_docs_navigator(deps.docs_navigator, paths_to_get)
            
            if leaf['requirement'] == "Deepwiki (Electron Framework)":
                # Special case for the top-level rubric item, assuming it's covered by the documentation itself.
                score = 1.0
                reasoning = "The rubric item 'Deepwiki (Electron Framework)' refers to the overall project, which is covered by the documentation as a whole."
                return leaf['path'], {
                    "score": score,
                    "reasoning": reasoning,
                    "evidence": "Overall project documentation",
                    "tokens": {"input": 0, "output": 0}
                }

            summarized_docs_tree = summarize_docs_tree(docs_tree)
            prompt = f"""
Evaluate this criteria against the documentation:

Criteria: "{leaf['requirement']}"

Here is a summary of the documentation tree:
```json
{json.dumps(summarized_docs_tree, indent=2)}
```

Here is some relevant content from the documentation:
```
{docs_content}
```

Based on the provided information, is the criteria documented? Respond with the exact JSON format specified.
""".strip()
            with open(config.LOG_FILE, "a") as f:
                f.write(f"--- PROMPT FOR {leaf['requirement']} ---\n")
                f.write(prompt)
                f.write("\n--- END PROMPT ---\n\n")
...
async def run(args):
    ...
    if args.reference == "deepwiki-agent":
        reference_dir = "deepwiki"
    docs_path = os.path.join(config.PROJECT_ROOT, "examples", "OpenHands", reference_dir)
