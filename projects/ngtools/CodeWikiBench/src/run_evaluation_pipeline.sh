...
if [[ "$SKIP_EVALUATION" == false ]]; then
    ...
    while [[ -n "$remaining_models" ]]; do
        ...
        eval_cmd="uv run python -m judge.judge --repo-name \"$REPO_NAME\" --model \"$model\" --batch-size $BATCH_SIZE --max-retries $MAX_RETRIES --reference \"$REFERENCE\""
...
if [[ "$SKIP_COMBINATION" == false ]] && [[ $model_count -gt 1 ]]; then
    ...
    combine_cmd="uv run python -m judge.combine_evaluations --repo-name \"$REPO_NAME\" --method \"$COMBINATION_METHOD\" --reference \"$REFERENCE\""
...
if [[ "$VISUALIZE" == true ]]; then
    ...
    if [[ -f "judge/visualize_evaluation.py" ]]; then
        uv run python -m judge.visualize_evaluation --repo-name "$REPO_NAME" --reference "$REFERENCE"
        if [[ $? -eq 0 ]]; then
