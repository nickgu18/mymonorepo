...
from .transport.gemini_cli import GeminiCLITransport
...
class InternalClient:
    ...
    async def process_query(
        ...
        if transport is not None:
            ...
            cli_path = str(configured_options.cli_path) if configured_options.cli_path else ""
            # Simple heuristic detection for Gemini
            is_gemini = "gemini" in cli_path.lower() or "gemini" in str(configured_options.extra_args).lower()
            
            if is_gemini:
                if not isinstance(prompt, str):
                    raise ValueError("Gemini transport currently only supports string prompts")
                chosen_transport = GeminiCLITransport(
                    prompt=prompt,
        ...
        try:
            ...
            async for data in query.receive_messages():
                # Allow transport to yield raw messages or do its own parsing?
                # parse_message expects dict matching Claude protocol
