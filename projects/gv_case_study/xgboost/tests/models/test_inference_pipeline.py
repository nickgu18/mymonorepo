...
def test_predict_batch_matches_model_feature_columns() -> None:
    ...
    enriched = base_frame.merge(exp_features, on="person_id", how="left")
    ...
    assert "industry" in preds.columns

    # Ranks should reset within each industry cohort, since the model
    # was trained with per-industry XGBRanker groups.
    for _, group in preds.groupby("industry"):
