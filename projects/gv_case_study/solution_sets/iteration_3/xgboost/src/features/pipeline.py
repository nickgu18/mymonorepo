...
class FeaturePipeline:
    ...
    def _build_founder_features(self, clean_data: RankingCleanData) -> pd.DataFrame:
        experience, founder_ids, founder_history = self._prepare_experience_and_founders(clean_data)
        ...
        for person_id, history in founder_history.groupby("person_id"):
            ...
            education_tier, education_level = self._education_features_for(str(person_id), edu_tier_lookup, edu_level_lookup)

            performance, industry = self._company_context_for(str(company_id), perf_lookup, industry_lookup)

            perf_aggs = self._aggregate_founder_performance(history, perf_lookup, str(company_id))
            row = self._assemble_founder_row(
                ...
                perf_aggs,
                list(self.cfg.selected_columns),
            )
            rows.append(row)
    ...
    def _aggregate_founder_performance(self, founder_history: pd.DataFrame, perf_lookup: dict, target_company_id: str) -> dict:
        founded_companies = founder_history.loc[founder_history["is_founder"] == True, "company_id"].dropna().astype(str).tolist()
        perfs = [perf_lookup.get(cid) for cid in founded_companies]
        vals = [float(p) for p in perfs if p is not None and not pd.isna(p)]
        out: dict = {}
        out["founder_has_perf"] = int(len(vals) > 0)
        out["founder_perf_mean"] = float(sum(vals) / len(vals)) if vals else float("nan")
        out["founder_perf_max"] = float(max(vals)) if vals else float("nan")
        # last performance: prefer target company performance, else latest available by history order
        last_perf = perf_lookup.get(str(target_company_id))
        if last_perf is None or pd.isna(last_perf):
            last_perf = vals[-1] if vals else float("nan")
        out["founder_perf_last"] = float(last_perf) if not pd.isna(last_perf) else float("nan")
        return out
    ...
    def _assemble_founder_row(
        ...
        for key, val in perf_aggs.items():
    ...
    def _empty_founder_frame(self) -> pd.DataFrame:
        ...
        for col in self.cfg.selected_columns:
