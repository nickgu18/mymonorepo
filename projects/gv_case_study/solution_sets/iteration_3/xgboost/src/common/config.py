...
def _materialize(data: Mapping[str, Any]) -> Config:
    ...
    outputs_cfg = OutputConfig(batch_predictions=to_path(data["outputs"]["batch_predictions"]))
    logging_cfg = LoggingConfig(
        level=str(data.get("logging", {}).get("level", "INFO")),
        format=str(data.get("logging", {}).get("format", "json")),
        redact_fields=tuple(data.get("logging", {}).get("redact_fields", []) or ()),
    )
    return Config(
        project=project_cfg,
        data=data_cfg,
