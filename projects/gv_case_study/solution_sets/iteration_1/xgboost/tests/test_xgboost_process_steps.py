...
def _attach_company_info_and_perf_features(
    ...
    for person_id, history in founder_history.groupby("person_id"):
        founded = history[history["is_founder"] == True]  # noqa: E712
        company_ids = founded["company_id"].dropna().astype(str).tolist()
        perfs = [perf_lookup.get(cid) for cid in company_ids]
        vals = [float(p) for p in perfs if p is not None and not math.isnan(p)]
        ...
        # otherwise fall back to the last available value.
        if not founded.empty:
            last_company = str(founded.sort_values("order").iloc[-1]["company_id"])
            last_perf_val = perf_lookup.get(last_company)
        else:
            last_perf_val = None
        if last_perf_val is None or (isinstance(last_perf_val, float) and math.isnan(last_perf_val)):
            last_perf_val = vals[-1] if vals else float("nan")
        founder_perf_last = float(last_perf_val) if not math.isnan(last_perf_val) else float("nan")
