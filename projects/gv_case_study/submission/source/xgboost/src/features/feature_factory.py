...
def compute_founder_experience_features(
    experience: pd.DataFrame,
    cut_off_years: Optional[Mapping[str, int]] = None,
    current_year: int = 2025,
) -> pd.DataFrame:
    """Aggregate founder experience history into count/duration features.

    This helper is designed to be used post-split with a per-founder
    snapshot year (e.g. label_company_founded_year). When cut_off_years
    is provided, only roles whose start_date year is <= the founder's
    cut-off year are kept, and open-ended tenures are truncated at
    31-Dec-cut_off_year.
    """
    ...
    if experience is None or experience.empty:
        return pd.DataFrame(columns=columns)

    work = experience.copy()
    work["person_id"] = work["person_id"].astype(str)
    work["start_date"] = pd.to_datetime(work.get("start_date"), errors="coerce")
    work["end_date"] = pd.to_datetime(work.get("end_date"), errors="coerce")

    # Ensure role-type flags exist and are numeric {0,1}.
    flag_cols = [
        "is_c_suite",
        "is_employee",
        "is_executive",
    ...
    cut_off_years_dict = dict(cut_off_years or {})
    fallback_end = pd.Timestamp(int(current_year), 12, 31)
    ...
    for person_id, history in work.groupby("person_id"):
        ...
        for row in history.itertuples(index=False):
            ...
            effective_end = end_date if pd.notna(end_date) else fallback_end

            if cut_year is not None:
                cut_end = pd.Timestamp(int(cut_year), 12, 31)
                if start_date > cut_end:
                    continue
                if effective_end > cut_end:
                    effective_end = cut_end

            if effective_end < start_date:
                duration_years = 0.0
            else:
                duration_days = (effective_end - start_date).days
                duration_years = max(duration_days / 365.25, 0.0)

            total_years += float(duration_years)
            n_roles += 1

            n_c_suite += int(getattr(row, "is_c_suite"))
            n_employee += int(getattr(row, "is_employee"))
            n_exec += int(getattr(row, "is_executive"))
            n_founder += int(getattr(row, "is_founder"))
            n_board += int(getattr(row, "is_on_board"))

        rows.append(
            {
                "person_id": person_id,
                "n_c_suite_roles": n_c_suite,
                "n_employee_roles": n_employee,
                "n_executive_roles": n_exec,
                "n_founder_roles": n_founder,
                "n_board_roles": n_board,
                "n_experience_roles": n_roles,
