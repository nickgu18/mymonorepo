...
description: >
 ...
 Usage: /legion-investigate <problem-description-or-file-path-or-issue-url>
user-invocable: true
...
Read the mission-order-template from `template.md` in the repo root. Structure the investigation body using that template.
...
(Fill every field from template.md — mark N/A if not applicable)
...
gh issue create \
 ...
 --title "<concise title>" \
 --label "$LABELS" \
 --body-file /tmp/legion-investigate-issue.md
```
